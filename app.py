                    ...
                    <form method="POST" action="/send-manual">
                        <div class="form-group">
                            ...
                            <select id="post_type" name="post_type" required>
                                ...
                                <option value="new_regulations">📋 Новое в НТД</option>
...
class SafetyContentManager:
    ...
    def _load_all_content(self):
        """Загрузка полного контента на 1 месяц"""
        return {
    ...
    def _load_daily_rules(self):
        """Правила дня - полный контент на 4 недели"""
        return {
...
<b>🎯 Ключевые точки внимания:</b> переезды, мосты, тоннели, станции, места работ""",
            ...
            # ... продолжаем для всех 20 дней
            5: """⚡ <b>ПРАВИЛО ДНЯ</b>

<b>ПТЭ п.12.11:</b> При отказе автотормозов в поезде машинист обязан немедленно привести в действие вспомогательный тормоз локомотива.

<b>📝 Алгоритм действий:</b>
1. Немедленно применить вспомогательный тормоз локомотива
2. Подать сигнал общей тревоги (один длинный, три коротких)
3. Доложить поездному диспетчеру
4. Принять меры к остановке на площадке

<b>⚠️ Важно:</b> Запрещается движение с неработающими автотормозами!"""
        }
...
<b>14 метров</b> - расстояние, которое проходит локомотив за 1 секунду при скорости 50 км/ч.

<b>📝 Расчет времени реакции:</b>
• 0.5-1 сек - обнаружение опасности
• 0.5 сек - принятие решения
• 0.5-1 сек - физическая реакция

<b>⚠️ Факторы, увеличивающие время реакции:</b> усталость, отвлечение внимание, ночное время"""
        }

    def _load_weekly_tasks(self):
        """Ситуационные задачи с подробными решениями"""
        return {
            1: {
                'question': """🚨 <b>СИТУАЦИОННАЯ ЗАДАЧА НЕДЕЛИ</b>

<b>Ситуация:</b> В пути следования у грузового поезда вы заметили, что стрелка манометра тормозной магистрали не возвращается в положение зарядного давления после торможения.

<b>❓ Ваши действия?</b>""",
                'answer': """✅ <b>ПРАВИЛЬНЫЙ ОТВЕТ:</b>

1. <b>Немедленно подать сигнал общей тревоги</b> (один длинный, три коротких свистка)
2. <b>Применить вспомогательный тормоз локомотива</b> для экстренного торможения
3. <b>Остановить поезд на площадке</b>, если это возможно
4. <b>Доложить поездному диспетчеру</b> о ситуации и месте остановки
5. <b>Осмотреть состав</b> на предмет разрыва тормозной магистрали
6. <b>Принять меры к устранению неисправности</b> или ограждению поезда
...
<b>⚠️ Запрещается:</b> Продолжать движение с неработающими автотормозами!"""
    ...
    def _load_tech_training(self):
        """Техническая подготовка с детальными объяснениями"""
        return {
            1: """🔧 <b>ТЕХНИЧЕСКАЯ ПОДГОТОВКА: ТЭМ2</b>

<b>Система управления РКСУ:</b> Контроллер машиниста имеет 25 позиций.

<b>📝 Правила работы с контроллером:</b>
• При переходе с позиции на позицию выдерживать паузу 2-3 секунды
• Это позволяет снизить броски тока и продлить жизнь аппаратуры
• Резкие переключения приводят к искрению и износу контактов

<b>⚡ Технические особенности:</b>
• Позиции 1-15 - пусковые, для разгона
• Позиции 16-25 - ходовые, для поддержания скорости
• На каждой позиции контролировать ток тяговых двигателей""",

            2: """🔧 <b>ТЕХНИЧЕСКАЯ ПОДГОТОВКА: 2ТЭ10М</b>

<b>Дизель 10Д100 - критические параметры контроля:</b>

• <b>Давление масла:</b> минимальное 1,2 кгс/см²
  - Ниже 1,0 - немедленная остановка!
  - Норма: 2,5-4,5 кгс/см²

• <b>Температура воды:</b> максимальная 90°C
  - Выше 95°C - аварийная ситуация
  - Норма: 75-85°C

• <b>Давление топлива:</b> 8-10 кгс/см²
  - Отклонения указывают на неисправность ТНВД"""
        }
...
<b>Синдром привыкания к опасности:</b> После 1000 безопасных поездок мозг начинает воспринимать риск как минимальный.

<b>⚠️ Опасность:</b> Ослабление бдительности, пренебрежение правилами

<b>✅ Методы борьбы:</b>
• Мысленное моделирование опасных ситуаций
• Регулярное напоминание о реальных происшествиях
• Постоянное обучение и тренировки"""
        }

    def _load_new_regulations(self):
        """Новые нормативные документы"""
        return {
            1: """📋 <b>НОВОЕ В НТД</b>

<b>Обновленная Инструкция по сигнализации (ИСИ)</b> введена в действие с 01.12.2024.

<b>📝 Основные изменения:</b>
• Уточнены сигналы, подаваемые при производстве маневров с использованием радиосвязи
• Введены новые таблицы сигналов для цифровых систем связи
• Уточнен порядок действий при отказе средств связи

<b>🎯 Обязательно:</b> Ознакомиться с новыми таблицами сигналов в обязательном порядке!"""
        }
...
<b>⚠️ Требования безопасности:</b> Не находиться в опасной зоне, использовать СИЗ"""
    ...
    def _load_express_tests(self):
        """Экспресс-тесты с вариантами ответов"""
        return {
            1: {
...
D) Продолжать движение до станции""",
    ...
    def setup_scheduler(self):
        ...
        try:
            ...
            schedule_config = {
                ...
                '20:00': ('new_regulations', '📋 Новое в НТД')  # Только по понедельникам
            }
    ...
    def _get_content_by_type(self, post_type: str):
        ...
        content_map = {
            ...
            'weekly_task': self.content_db['weekly_tasks'].get(1)['question'],
            ...
            'new_regulations': self.content_db['new_regulations'].get(1),
...
def test_all_content():
    ...
    try:
        ...
        post_types = ['daily_rule', 'safety_number', 'weekly_task', 'tech_training', 
                     'incident_analysis', 'psychology', 'new_regulations', 'assistant_duties']
